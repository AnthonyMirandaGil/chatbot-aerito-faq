...
class FallbackResponseSelector(IntentClassifier):
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        if is_fallback_classifier_prediction(message.data):
            return
            
        if not self._should_fallback_response_selector(message):
            return
    ...
    def _should_fallback_response_selector(self, message: Message) -> bool:
        ...
        if intent_name !=  retrieval_intent:
            return False
...
def is_fallback_classifier_prediction(prediction: Dict[Text, Any]) -> bool:
    ...
    """
    print('INTENT_NAME_KEY: ', prediction.get(INTENT, {}).get(INTENT_NAME_KEY))
    print('DEFAULT_NLU_FALLBACK_INTENT_NAME', DEFAULT_NLU_FALLBACK_INTENT_NAME)
    return (
